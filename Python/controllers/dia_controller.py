...
def criar_dia(request: Request, ciclo_id: int, dia: DiaRequest, db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
...
@limiter.limit("30/minute")
def criar_dias_lote(request: Request, ciclo_id: int, dias: List[DiaRequest], db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
...
@limiter.limit("30/minute")
def sincronizar_dias_lote(request: Request, ciclo_id: int, dias: List[DiaRequest], db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
    return dia_service.sincronizar_dias_lote(db, ciclo_id, dias, user_id)


@router.put("/dias/{dia_id}", response_model=DiaResponse)
@limiter.limit("30/minute")
def atualizar_dia(request: Request, dia_id: int, dia: DiaRequest, db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
...
def remover_dia(request: Request, dia_id: int, db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
