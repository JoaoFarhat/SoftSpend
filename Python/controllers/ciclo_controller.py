...
def criar_ciclo(request: Request, ciclo: CicloRequest, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
@limiter.limit("60/minute")
def get_ciclo(request: Request, ciclo_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    return ciclo_service.get_user_ciclo_by_id(db, ciclo_id, user_id)


@router.delete("/ciclos/{ciclo_id}", status_code=204)
@limiter.limit("20/minute")
def delete_ciclo(request: Request, ciclo_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
...
@limiter.limit("20/minute")
def update_ciclo(request: Request, ciclo_id: int, ciclo_request: CicloRequest, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
